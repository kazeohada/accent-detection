...
def main():
    ...
    parser.add_argument(
        '--network-type', '-n',
        required = True,
        choices = ['cnn', 'multiclass', 'multilabel1', 'multilabel2', 'multilabel1l', 'multilabel2l'],
        help = 'network type')
    parser.add_argument(
        '--epochs',
        type = int,
    ...
    if args.evaluate:
        config = get_config()
        evaluate = config[args.network_type]['eval']
        output_dir = join(config['model-dir'], nn.__name__)
        evaluate(nn, folds, args.evaluate, output_dir)
    else:
