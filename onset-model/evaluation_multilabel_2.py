...
def write_audacity_labels(name, onsets, accents):
    if not os.path.exists(f"results/multilabel2/{name}"):
        os.makedirs(f"results/multilabel2/{name}")

    with open(f"results/multilabel2/{name}/guess_onset.txt", "w") as file:
        for o in onsets:
            file.write(f"{o}\t{o}\n")
    
    with open(f"results/multilabel2/{name}/guess_accent.txt", "w") as file:
        for a in accents:
            file.write(f"{a}\t{a}\n")
...
def evaluate_audio_sample(nn, model, d, epoch):
    ...
    a_ons_guess, a_acc_guess = nn.postprocess_y(y_ons_guess, y_acc_guess, 0.5, 0.5)
...
def evaluate_fold(nn, model, fold, epoch):
    for audio_sample in fold:
        ...
        yield evaluate_audio_sample(nn, model, audio_sample, epoch)
...
def evaluate(nn, folds, int_range, output_dir):
    eval_df = pd.DataFrame(columns=['epoch', 
                                'non-accent precision', 'non-accent recall', 'non-accent f-score',
    ...
    print("Evaluation for all labels")
    print(all_print)
    print("\nEvaluation of detection for non-accented onsets")
    print(ons_print)
    print("\nEvaluation of detection of accented onsets")
    print(acc_print)

    eval_df.to_csv("results/eval_multilabel2.csv", sep=",")
