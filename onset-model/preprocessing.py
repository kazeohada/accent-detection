...
# audio.
#
# y: A numpy array of annotations. [n][0] = 1 if the corresponding 10 ms of
# audio contains an onset and [n][0] = 0 otherwise. [n][1] = 1 if the corresponding 10 ms of
# audio contains an onset and [n][1] = 0 otherwise.
#
# a: A numpy array containing the onsets & accent time offsets. [accents[], onsets[]]
#
...
def list_annotation_files(data_dir):
    ann_dir = join(data_dir, 'annotations')
    return [[join(ann_dir, d, f) for f in sorted(listdir(join(ann_dir, d)), reverse=True)] for d in sorted(listdir(ann_dir))]
...
def preprocess_data(nn, seed, dataset_dir):
    ...
    A = [[np.loadtxt(f[0], ndmin=1), np.loadtxt(f[1], ndmin=1)] for f in ann_files]
    Y = [nn.preprocess_y(a, len(x)) for (a, x, n) in zip(A, X, N)] 
